  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // Refresh users (lightweight) when results change
        const ids = [...new Set((res?.results || []).map((t) => t.user_id).filter(Boolean))];
        if (ids.length) {
          ...
          await Promise.all(ids.map(async (id) => {
            ...
            try {
              ...
              map.set(id, u?.name || u?.email || `#${id}`);
            } catch {}
          }));
          if (!ignore) setUsersMap(map);
        }
      } catch (e) {
        if (ignore) return;
                ...
                <tbody>
                  {rows.map((t) => (
                    <tr key={t.id} className="border-t">
                      ...
                      <td className="py-2">{usersMap.get(t.user_id) || t.user_id || "-"}</td>
                      <td className="py-2">
                        <span
                  ...
                  {rows.length === 0 && (
                    <tr>
                      <td colSpan={12} className="py-6 text-center text-gray-500">No transactions found.</td>
                    </tr>
                  )}
                </tbody>
              </table>
